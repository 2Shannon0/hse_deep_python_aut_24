...
from cust_decorator import add, check_str
...
class TestDecarator(unittest.TestCase):
    ...
    @patch('sys.stdout', new_callable=StringIO)
    def test3_check_str(self, mock_stdout):
    ...
    @patch('sys.stdout', new_callable=StringIO)
    def test4_check_str(self, mock_stdout):

        check_str(None)

        expected_output = (
             'run "check_str" with positional args = (None,), '
             'attempt = 1, exception = ValueError\n'
             'run "check_str" with positional args = (None,), '
             'attempt = 2, exception = ValueError\n'
             'run "check_str" with positional args = (None,), '
